...
def parse_spec(spec: str):
    ...
    parts = spec.split("x")
    if len(parts) < 2:
        raise ValueError(f"Invalid spec '{spec}': expected '<shape>x<dtype>' (rank-0 not allowed).")
    dtype_token = parts[-1].lower()
    dims_strs = parts[:-1]
    if dtype_token not in SUPPORTED_DTYPES:
        raise ValueError(f"Unsupported element type '{dtype_token}' in '{spec}'. "
...
def main(argv):
    ...
    for idx, spec in enumerate(specs):
        try:
            ...
            print(f"Created {out_name}: shape={tuple(shape)} dtype={arr.dtype} "
